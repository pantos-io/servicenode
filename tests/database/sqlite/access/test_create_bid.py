...
@unittest.mock.patch('pantos.servicenode.database.access.get_session_maker')
def test_create_bid_hub_added_by_another_thread(mocked_session,
    ...
    assert mocked_session().close.called_once_with()


@unittest.mock.patch('pantos.servicenode.database.access._read_hub_contract')
@unittest.mock.patch('pantos.servicenode.database.access.get_session_maker')
def test_create_bid_error(mocked_session, mocked_read_hub_contract,
                          embedded_db_session_maker):
    mocked_session.return_value = embedded_db_session_maker
    mocked_read_hub_contract.side_effect = Exception()
